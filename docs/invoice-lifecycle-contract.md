...
- `slimweb_invoices_create`: POST `/commerce/invoices`. Creates or repairs an explicit draft_id (only unissued or definitively failed) with stable `idempotency_key`, optional `order_id`, or standalone `transaction_date`, `buyer`, `items`.
...
2. Release the verified Core commit as `v0.1.9` (package version and lockfile version together). Do not move the existing v0.1.7 tag.
3. In each MCP consumer run `npm install --save-exact github:ericchen1972/SlimWeb-MCP-Core#v0.1.9` and commit both package.json and package-lock.json. Never publish local file dependencies or manually fabricate a lockfile SHA.
4. Run Core and both consumer Node suites against that exact installed tag. Candidate contract: 137 tools, SHA256 `a6ac0a1043ce6fb0fd6999338495e919fc1faa3d1140190d903e04f6b3b63e85`.
...
Local consumer verification temporarily copies candidate Core source into the installed package, then restores the original v0.1.7 installation. Release manifests now pin v0.1.9 and the lockfile resolves its immutable Git commit. Standalone hides the nine invoice tools for older full-contract installations lacking `invoice_lifecycle_v1`.
